...
namespace ProjectAllocation.Controls
{
    ...
    /// </summary>
    [ToolboxBitmap(typeof(ProjectAllocation.Controls.XPLabel), "XPLabel.bmp")]
	public class XPLabel : System.Windows.Forms.Control
	{
		...
		private TextAlignStyle _taStyle = TextAlignStyle.AlignLeftMiddle;
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			SizeF stringSize = e.Graphics.MeasureString(Text, Font, Width);
			e.Graphics.DrawString(Text, Font, tb, CalculateTextLocation(stringSize));
			tb.Dispose();
			...
			if( ShowShadow )
			{
				...
				e.Graphics.DrawString(Text, Font, tb1, CalculateTextLocation(stringSize).X + 0.5f, CalculateTextLocation(stringSize).Y + 0.5f);
				tb1.Dispose();
			}
		}
        ...
        private PointF CalculateTextLocation(SizeF stringSize)
		{
			float x = 0.0f, y = 0.0f;

			switch(TextAlign)
			{
				case TextAlignStyle.AlignLeftTop:
					x = 2;
					y = 0;
					break;
				case TextAlignStyle.AlignLeftBottom:
					x = 2;
                    y = Height - stringSize.Height;
					break;
				case TextAlignStyle.AlignLeftMiddle:
					x = 2;
                    y = (Height - stringSize.Height) / 2;
					break;
				case TextAlignStyle.AlignMiddleTop:
                    x = (Width - stringSize.Width) / 2;
					y = 0;
					break;
				case TextAlignStyle.AlignMiddleMiddle:
                    x = (Width - stringSize.Width) / 2;
                    y = (Height - stringSize.Height) / 2;
					break;
				case TextAlignStyle.AlignMiddleBottom:
                    x = (Width - stringSize.Width) / 2;
                    y = (Height - stringSize.Height);
					break;
				default:
